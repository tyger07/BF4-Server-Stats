...
if(isset($_GET['server']) AND !empty($_GET['server']))
{
	...
	$query  = "SELECT `MinPlayers`, `MaxPlayers`, `AvgPlayers` FROM `tbl_mapstats` WHERE ServerID = ". $id ." LIMIT {$limit}";
	$result = mysqli_query($BF4stats, $query);
}
// this must be a global stats page
else
{
	$query  = "SELECT `MinPlayers`, `MaxPlayers`, `AvgPlayers` FROM `tbl_mapstats` WHERE 1 LIMIT {$limit}";
	$result = mysqli_query($BF4stats, $query);
}
...
, "R"=>150, "G"=>150, "B"=>150);
$myPicture->drawText(297,18,"Minimum, maximum and average players, last ". $limit ." rounds",$TextSettings);
