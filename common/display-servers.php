...
// no server name has been established...  is this a global page?
...
foreach($ServerIDs as $this_ServerID)
{
	...
	if($this_ServerID != $ServerID)
	{
		...
		// change display in menu if this is the current server selected
		echo '<option value="' . $this_ServerID . '">' . $Server_Name . '</option>';
	}
}

// is there more than one server?
// show global stats option at bottom of the form if there is more than one server and we aren't already viewing the global stats page
if(count($ServerIDs) > 1 && !(empty($ServerID) && !empty($page)))
{
	// show global stats in form
	echo '
...
if(!empty($ServerID))
{
	echo '<a href="' . $battlelog . '" target="_blank"><img src="./images/joinbtn.png" alt="join" class="imagebutton"/></a>';
}
